...
qx.Class.define("zx.test.io.remote.RemoteThinXhrFeature", {
  ...
  construct() {
    ...
    body.add(
      <p>
        This thin client should be able to see changes made by the Desktop-based
        demo runner; the demo runner will perform tests and watch the console
        output of this iframe to check that the updates happen
      </p>
    );
    body.add(
      <p>
        Unlike the zx.test.io.remote.DemoRemoteWindows, both the Desktop and the
        thin client are independently talking to the server via Xhr and not
        using postMessage between windows
      </p>
    );
  ...
  members: {
      ...
      switch (id) {
        case "btnChangeAge":
          ...
          btn.addListener("execute", async () => {
            ...
            let controller =
              await qx.core.Init.getApplication().getNetController();
            controller.flush();
          });
          return btn;
    ...
    async _onReady(options) {
      ...
      let grandad = (this._grandad = controller.getUriMapping(
