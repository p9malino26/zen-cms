...
qx.Class.define("zx.ui.tree.simple.Node", {
  ...
  members: {
    ...
    _onChildrenChange: function (evt) {
      ...
      function added(lst) {
        for (var i = 0; i < lst.length; i++) lst[i].set({ parent: t });
      }
      function removed(lst) {
        for (var i = 0; i < lst.length; i++) lst[i].set({ parent: null });
      }

      if (data.type == "add") added(data.added);
      else if (data.type == "remove") removed(data.removed);
