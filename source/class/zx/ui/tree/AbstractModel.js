...
qx.Class.define("zx.ui.tree.AbstractModel", {
  ...
  members: {
    ...
    _addNode: function (node, parentNode) {
      if (qx.core.Environment.get("qx.debug"))
        this.assert(
          ...
          "Node " + node + " is already part of the model"
        );
      ...
      this.__nodeData[node.toHashCode()] = {
        ...
        childrenListener: children.addListener(
          "change",
          this._onNodeChildrenChange,
          this
        )
      };
    ...
    _setNodeParent: function (node, parentNode) {
      var nodeData = this.__nodeData[node.toHashCode()];
      if (!nodeData && !parentNode) return;
      if (!nodeData && parentNode) {
        this._addNode(node, parentNode);
        var children = this.getChildrenObject(parentNode);
        if (!children.contains(node)) children.push(node);
        return;
      } else if (nodeData.parentNode == parentNode) return;
    ...
    getChildrenObject: function (parent) {
      throw new Error(
        this.classname + ".getChildrenObject() is not implemented"
      );
    },
    ...
    moveTo: function (child, parent, after) {
      ...
      if (arr.indexOf(after) == fromIndex - 1) return;

      arr.splice(fromIndex, 1);
      if (!after) arr.unshift(child);
      else {
        var toIndex = arr.indexOf(after) + 1;
        if (toIndex == arr.length) arr.push(child);
