...
qx.Class.define("zx.utils.Target", {
  ...
  members: {
    ...
    _applyValue(value, oldValue) {
      if (this.__callback) this.__callback(value, oldValue);
    },
  ...
  statics: {
    bindEvent(object, path, eventName, cb, context) {
      var target = new zx.utils.Target(function (value, oldValue) {
        if (oldValue) oldValue.removeListener(eventName, cb, context);
        if (value) value.addListener(eventName, cb, context);
      });
